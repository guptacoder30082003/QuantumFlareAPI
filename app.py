...
image_model = None
photodiode_model = None

def load_models():
    global image_model, photodiode_model
    try:
        ...
        photodiode_model = tf.keras.models.load_model('models/model2.h5')
...
def preprocess_image_with_opencv(base64_image):
    ...
    microplastic_count = count_microplastics_opencv(original_rgb)
    ...
    return image_batch, microplastic_count

def preprocess_photodiode_data(photodiode_data):
    photodiode_array = np.array(photodiode_data, dtype=np.float32)
    photodiode_batch = np.expand_dims(photodiode_array, axis=0)
    return photodiode_batch

def combine_predictions(image_pred, photodiode_pred, image_weight=0.7, photodiode_weight=0.3):
    combined_pred = (image_weight * image_pred) + (photodiode_weight * photodiode_pred)
...
def classify_microplastic():
    try:
        ...
        processed_image, microplastic_count = preprocess_image_with_opencv(image_data)
        image_prediction = image_model.predict(processed_image, verbose=0)
        
        processed_photodiode = preprocess_photodiode_data(photodiode_data)
        photodiode_prediction = photodiode_model.predict(processed_photodiode, verbose=0)
        
        combined_prediction = combine_predictions(image_prediction, photodiode_prediction)
...
def health_check():
    models_loaded = image_model is not None and photodiode_model is not None
    return jsonify({
